...
class LazyRBST(Generic[T, F]):
    ...
    def _propagate(self, node: Node) -> None:
        ...
        if node.lazy != self.id:
            ...
            if node.left:
                ...
                node.left.lazy = (
                    lazy
                    if node.left.lazy == self.id
                    else self.composition(lazy, node.left.lazy)
                )
            if node.right:
                ...
                node.right.lazy = (
                    lazy
                    if node.right.lazy == self.id
                    else self.composition(lazy, node.right.lazy)
                )
            node.lazy = self.id
    ...
    def split(self, k: int) -> tuple["LazyRBST", "LazyRBST"]:
        ...
        return (
            LazyRBST(
                [], self.op, self.mapping, self.composition, self.e, self.id, _root=left
            ),
            LazyRBST(
                [],
                self.op,
    ...
    def insert(self, k: int, key: T) -> None:
        ...
        self.root = self._merge_node(
            self._merge_node(s, LazyRBST.Node(key, self.id)), t
        )
