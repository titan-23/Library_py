...
class ReversibleLazyAVLTree(Generic[T, F]):
    ...
    def split(self, k: int) -> tuple["ReversibleLazyAVLTree", "ReversibleLazyAVLTree"]:
        ...
        ), ReversibleLazyAVLTree(
            [], self.op, self.mapping, self.composition, self.e, self.id, r
        )

    def insert(self, k: int, key: T) -> None:
        s, t = self._split_node(self.root, k)
        self.root = self._merge_with_root(
            s, ReversibleLazyAVLTree.Node(key, self.id), t
        )
        ...
        while stack:
            if isinstance(stack[-1], tuple):
                ...
                if l <= left and right < r:
                    ...
                    node.lazy = (
                        f if node.lazy == self.id else self.composition(f, node.lazy)
                    )
                else:
    ...
    def all_apply(self, f: F) -> None:
        ...
        self.root.lazy = (
            f if self.root.lazy == self.id else self.composition(f, self.root.lazy)
        )
