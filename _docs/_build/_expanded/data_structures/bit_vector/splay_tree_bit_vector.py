...
class SplayTreeBitVector(BitVectorInterface):
    ...
    def _update(self, node: int) -> None:
        ...
        self.total[node] = (
            SplayTreeBitVector._popcount(self.key[node])
            + self.total[lnode]
            + self.total[rnode]
        )
    ...
    def pop(self, k: int = -1) -> int:
        assert 0 <= k < len(self), f"IndexError: SplayTreeBitVector.pop({k})"
        root = self._kth_elm_splay(self.root, k)
        size, child, key, bit_len, total = (
            self.size,
            self.child,
            self.key,
            self.bit_len,
        ...
        if bit_len[root] == 1:
            ...
            key[root] = ((v >> (bit_len[root] - k)) << ((bit_len[root] - k - 1))) | (
                v & ((1 << (bit_len[root] - k - 1)) - 1)
            )
            bit_len[root] -= 1
            size[root] -= 1
            total[root] -= res
