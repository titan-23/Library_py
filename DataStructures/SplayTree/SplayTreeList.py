...
class SplayTreeListData(Generic[T]):
...
class SplayTreeList(Generic[T]):
  ...
  def __init__(self,
               data: SplayTreeListData,
               n_or_a: Union[int, Iterable[T]]=0,
               _root: int=0
               ) -> None:
    self.data: SplayTreeListData = data
    self.root: int = _root
    if not n_or_a:
      return
  ...
  def split(self, k: int) -> Tuple['SplayTreeList', 'SplayTreeList']:
    assert -len(self) < k <= len(self), \
        f'IndexError: SplayTreeList.split({k}), len={len(self)}'
    if k < 0: k += len(self)
    if k >= self.data.arr[self.root<<2|2]:
      return self, SplayTreeList(self.data, _root=0)
    self.root = self._kth_elm_splay(self.root, k)
    left = SplayTreeList(self.data, _root=self.data.arr[self.root<<2])
  ...
  def insert(self, k: int, key: T) -> None:
    assert -len(self) <= k <= len(self), \
        f'IndexError: SplayTreeList.insert({k}, {key}), len={len(self)}'
    if k < 0: k += len(self)
    data = self.data
  ...
  def pop(self, k: int=-1) -> T:
    assert -len(self) <= k < len(self), \
        f'IndexError: SplayTreeList.pop({k})'
    data = self.data
    if k == -1:
      node = self._right_splay(self.root)
  ...
  def __setitem__(self, k: int, key: T):
    assert -len(self) <= k < len(self), f'IndexError: SplayTreeList.__setitem__({k})'
    self.root = self._kth_elm_splay(self.root, k)
    self.data.keys[self.root] = key
    self.data._update(self.root)

  def __getitem__(self, k: int) -> T:
    assert -len(self) <= k < len(self), f'IndexError: SplayTreeList.__getitem__({k})'
    self.root = self._kth_elm_splay(self.root, k)
