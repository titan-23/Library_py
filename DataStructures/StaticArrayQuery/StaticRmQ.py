...
class StaticRmQ():
  ...
  class SparseTableRmQ():
    ...
    def __init__(self, a: List[int], e: int):
      self.size = len(a)
  ...
  def __init__(self, a: Iterable[int], e=10**9):
    self.INF = e
    a = list(a)
    n = len(a)
    ...
    bucket_data = StaticRmQ.SparseTableRmQ([min(b) for b in bucket], self.INF)
    bucket_bit = [array('I', bytes(4*bucket_size)) for _ in range(bucket_cnt)]
    ...
    for i, b in enumerate(bucket):
      ...
      for j, e in enumerate(b):
        ...
        stack.append(j)
        if g == -1: continue
        bucket_bit[i][j] = bucket_bit[i][g] | (1 << g)
    self.n = n
