...
class PersistentArray32(Generic[T]):
    ...
    class Node:
        ...
        def copy(self) -> "PersistentArray32.Node":
            return PersistentArray32.Node(
                self.key, None if self.child is None else self.child[:]
            )

    def __init__(
        self, a: Iterable[T] = [], _root: Optional["PersistentArray32.Node"] = None
    ):
    ...
    def _build(self, a: Iterable[T]) -> Optional["PersistentArray32.Node"]:
        ...
        def dfs(indx: int, dep: int) -> Node:
            if indx >= n:
                return None
            node = (
                Node(a[indx]) if indx + (1 + 1) * beki[dep] < n else Node(a[indx], None)
            )
            for i in range(_titan23_PersistentArray32_M):
                node.child[(i + 1) & _titan23_PersistentArray32_MSK] = dfs(
                    indx + (i + 1) * beki[dep], dep + 1
                )
            return node
