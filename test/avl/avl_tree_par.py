...
class AVLTreeIterator:
    ...
    def __init__(self, tree: AVLTree, it: int) -> None:
    ...
    def _next(self) -> Optional[int]:
        ...
        return (
            now
            if flag and pre is self.tree.left[now]
            else self._min(self.tree.right[now])
        )
    ...
    def _prev(self) -> Optional[int]:
        ...
        return (
            now
            if flag and pre is self.tree.right[now]
            else self._max(self.tree.left[now])
        )
...
class AVLTreeSet(Generic[T]):
    ...
    def _update(self, node: int) -> None:
        ...
        self.height[node] = 1 + max(
            self.height[self.left[node]], self.height[self.right[node]]
        )
        ...
        while node:
            ...
            if b == 2:
                new_node = (
                    ...
                    if self._balance(left[node]) == -1
                    else self._rotate_right(node)
                )
            elif b == -2:
                new_node = (
                    self._rotate_RL(node)
                    if self._balance(right[node]) == 1
                    else self._rotate_left(node)
                )
            elif b != 0:
                node = par[node]
                break
