# from titan_pylib.data_structures.b_tree.gomi.BTreeMultiset import BTreeMultiset
...
class BTreeMultiset(Generic[T]):
    ...
    class Node:
        ...
        def split(self, i: int) -> "BTreeMultiset.Node":
            right = BTreeMultiset.Node()
            self.key, right.key = self.key[:i], self.key[i:]
            self.child, right.child = self.child[: i + 1], self.child[i + 1 :]
            self.cnt, right.cnt = self.cnt[: i + 1], self.cnt[i + 1 :]
        ...
        def insert_child(self, i: int, node: "BTreeMultiset.Node", size=-1) -> None:
        ...
        def append_child(self, node: "BTreeMultiset.Node") -> None:
            self.size += node.size
        ...
        def pop_child(self, i: int = -1, size: int = -1) -> "BTreeMultiset.Node":
    ...
    def _is_over(self, node: "BTreeMultiset.Node") -> bool:
    ...
    def _discard_right(self, node: "BTreeMultiset.Node") -> T:
        stack = []
    ...
    def _discard_left(self, node: "BTreeMultiset.Node") -> T:
        stack = []
        while not node.is_leaf():
            stack.append(node)
    ...
    def _merge(self, node: "BTreeMultiset.Node", i: int) -> "BTreeMultiset.Node":
        y = node.child[i]
    ...
    def _merge_key(self, key: T, node: "BTreeMultiset.Node", i: int) -> None:
    ...
    def _discard(self, key: T, node: Optional["BTreeMultiset.Node"] = None) -> bool:
        if node is None:
            node = self._root
        if not node.key:
            return False
        ...
        while True:
            ...
            if node.child[i].len_key() == self._m // 2:
                if (
                    i + 1 < len(node.child)
                    and node.child[i + 1].len_key() > self._m // 2
                ):
    ...
    def __repr__(self):
        return f"BTreeMultiset({self.tolist()})"
