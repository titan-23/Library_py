...
class DynamicWaveletMatrix(WaveletMatrix):
  ...
  def insert(self, k: int, x: int) -> None:
    """位置 ``k`` に ``x`` を挿入します。 :math:`O(\\log{(n)}\\log{(\\sigma)})` です。
    """
  ...
  def pop(self, k: int) -> int:
    """位置 ``k`` の要素を削除し、その値を返します。 :math:`O(\\log{(n)}\\log{(\\sigma)})` です。
    """
  ...
  def update(self, k: int, x: int) -> None:
    """位置 ``k`` の要素を ``x`` に更新します。 :math:`O(\\log{(n)}\\log{(\\sigma)})` です。
    """
