from Library_py.data_structures.SegmentTree.SegmentTreeInterface import SegmentTreeInterface
...
class SegmentTree:
  ...
  def __init__(self, a, op, e):
    ...
    Args:
      n_or_a (Union[int, Iterable[T]]): 要素数または列です

      op (Callable[[T, T], T]): 二項演算の関数です

      e (T): 単位元です

    Examples:
      長さ `n` の区間 `min` セグ木を作るときは以下のようにします

      >>> n = int(input())
      >>> op = lambda s, t: min(s, t)
      >>> e = INF
      >>> seg = SegmentTree(n, op, e)
