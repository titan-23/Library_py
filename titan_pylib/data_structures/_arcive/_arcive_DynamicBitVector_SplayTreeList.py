...
class DynamicBitVector_SplayTreeList():
  ...
  def __init__(self, data: 'DynamicBitVector_SplayTreeList_Data', n_or_a: Union[int, Iterable[int]]=0, _root: int=0):
    self.data = data
    self.root = _root
    if not n_or_a:
      return
  ...
  def _update_triple(self, x: int, y: int, z: int) -> None:
    ...
    keydata[x<<1|1] = keydata[lx<<1|1] + keydata[x<<1] + keydata[rx<<1|1]
    keydata[y<<1|1] = keydata[ly<<1|1] + keydata[y<<1] + keydata[ry<<1|1]
  ...
  def _update(self, node: int) -> None:
    ...
    keydata[node<<1|1] = keydata[lnode<<1|1] + keydata[node<<1] + keydata[rnode<<1|1]
  ...
  def split(self, k: int) -> Tuple['DynamicBitVector_SplayTreeList', 'DynamicBitVector_SplayTreeList']:
    ...
    left = DynamicBitVector_SplayTreeList(self.data, _root=self.data.arr[self.root<<2])
