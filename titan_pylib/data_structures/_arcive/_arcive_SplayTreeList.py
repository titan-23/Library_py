...
class SplayTreeList(Generic[T]):
    ...
    def _update(self, node: Node) -> None:
        node.size = (
            1
            + (0 if node.left is None else node.left.size)
        ...
        for _ in range(len(path) >> 1):
            ...
            if di & 1 == di >> 1 & 1:
                if di & 1:
                    ...
                    self._update(pnode)
                    node.size = (
                        1 + (0 if node.left is None else node.left.size) + pnode.size
                    )
                    tmp.size = (
                        1 + (0 if tmp.left is None else tmp.left.size) + node.size
                    )
                else:
                    tmp = node.right
                    node.right = tmp.left
                    ...
                    self._update(pnode)
                    node.size = (
                        1 + pnode.size + (0 if node.right is None else node.right.size)
                    )
                    tmp.size = (
                        1 + node.size + (0 if tmp.right is None else tmp.right.size)
                    )
            else:
                if di & 1:
                    tmp = node.left
