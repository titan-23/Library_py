...
class WBTreeSet(OrderedSetInterface, Generic[T]):
    ...
    def pop_max(self) -> T:
        assert (
            self.root
        ), f"IndexError: {self.__class__.__name__}.pop_max(), pop_max from Empty {self.__class__.__name__}"
        return self.pop()

    def pop_min(self) -> T:
        assert (
            self.root
        ), f"IndexError: {self.__class__.__name__}.pop_min(), pop_min from Empty {self.__class__.__name__}"
        return self.pop(0)
