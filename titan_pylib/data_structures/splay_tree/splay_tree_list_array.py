...
class SplayTreeListArrayData(Generic[T]):
    ...
    def _update(self, node: int) -> None:
        arr = self.arr
        arr[node << 2 | 2] = (
            1 + arr[arr[node << 2] << 2 | 2] + arr[arr[node << 2 | 1] << 2 | 2]
        )
...
class SplayTreeListArray(Generic[T]):
    ...
    def pop(self, k: int = -1) -> T:
        assert -len(self) <= k < len(self), f"IndexError: SplayTreeListArray.pop({k})"
        data = self.data
        if k == -1:
            node = self._right_splay(self.root)
    ...
    def __setitem__(self, k: int, key: T):
        assert (
            -len(self) <= k < len(self)
        ), f"IndexError: SplayTreeListArray.__setitem__({k})"
        self.root = self._kth_elm_splay(self.root, k)
    ...
    def __getitem__(self, k: int) -> T:
        assert (
            -len(self) <= k < len(self)
        ), f"IndexError: SplayTreeListArray.__getitem__({k})"
        self.root = self._kth_elm_splay(self.root, k)
