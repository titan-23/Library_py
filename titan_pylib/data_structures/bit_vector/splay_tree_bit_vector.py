...
class SplayTreeBitVector(BitVectorInterface):
  ...
  def _update(self, node: int) -> None:
    ...
    self.total[node] = SplayTreeBitVector._popcount(self.key[node]) + self.total[lnode] + self.total[rnode]
  ...
  def insert(self, k: int, key: int) -> None:
    assert 0 <= k <= len(self), \
        f'IndexError: SplayTreeBitVector.insert({k}, {key}), len={len(self)}'
    if not self.root:
      node = self._make_node(key, 1)
      self.root = node
      return
    bit_len, child, size, keys, total = self.bit_len, self.child, self.size, self.key, self.total
    if k == size[self.root]:
      node = self._right_splay(self.root)
      if bit_len[node] == self.w:
        v = keys[node] << 1 | key
        new_node = self._make_node(v&1, 1)
        keys[node] = v>>1
        ...
        v = keys[node]
        bl = k - bit_len[node] - size[child[node<<1]]
      ...
      node = self._kth_elm_splay(self.root, k)
      if bit_len[node] == self.w:
        k -= size[child[node<<1]]
        v = keys[node]
        bl = bit_len[node] - k
        v = (((v >> bl) << 1 | key) << bl) | (v & ((1<<bl)-1))
        new_node = self._make_node(v>>self.w, 1)
        ...
        v = keys[node]
        bl = bit_len[node] - k + size[child[node<<1]]
        keys[node] = (((v >> bl) << 1 | key) << bl) | (v & ((1<<bl)-1))
        bit_len[node] += 1
        size[node] += 1
        total[node] += key
  ...
  def pop(self, k: int=-1) -> int:
    assert 0 <= k < len(self), \
        f'IndexError: SplayTreeBitVector.pop({k})'
    root = self._kth_elm_splay(self.root, k)
    size, child, key, bit_len, total = self.size, self.child, self.key, self.bit_len, self.total
    k -= size[child[root<<1]]
    v = key[root]
    res = v >> (bit_len[root] - k - 1) & 1
    if bit_len[root] == 1:
      if not child[root<<1]:
      ...
      key[root] = ((v >> (bit_len[root]-k)) << ((bit_len[root]-k-1))) | (v & ((1<<(bit_len[root]-k-1))-1))
      bit_len[root] -= 1
      size[root] -= 1
      total[root] -= res
      self.root = root
  ...
  def _pref(self, r: int) -> int:
    assert 0 <= r <= len(self), \
        f'IndexError: SplayTreeBitVector._pref({r}), len={len(self)}'
    if r == 0:
      return 0
    if r == len(self):
    ...
    return self.total[self.root] - SplayTreeBitVector._popcount(self.key[self.root] & ((1<<(self.bit_len[self.root]-r))-1)) - self.total[self.child[self.root<<1|1]]
