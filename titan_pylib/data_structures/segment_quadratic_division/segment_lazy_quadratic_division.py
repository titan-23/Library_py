...
class SegmentLazyQuadraticDivision(Generic[T, F]):
    ...
    def __init__(
        ...
        self.data = [
            a[k * self.bucket_size : (k + 1) * self.bucket_size]
    ...
    def apply(self, l: int, r: int, f: F) -> None:
        ...
        if k1 == k2:
            ...
            if k1 < self.bucket_cnt:
                if l == 0:
                    self.bucket_lazy[k1] = (
                        ...
                        else self.composition(f, self.bucket_lazy[k1])
                    )
            ...
            for i in range(k1 + 1, k2):
                self.bucket_lazy[i] = (
                    f
                    if self.bucket_lazy[i] == self.id
                    else self.composition(f, self.bucket_lazy[i])
                )
            ...
            if k2 < self.bucket_cnt:
                if r == len(self.data[k2]):
                    self.bucket_lazy[k2] = (
                        ...
                        else self.composition(f, self.bucket_lazy[k2])
                    )
    ...
    def all_apply(self, f: F) -> None:
        ...
        """
        self.bucket_lazy = [
            f if bl == self.id else self.composition(f, bl) for bl in self.bucket_lazy
        ]

    def _propagate(self, k: int) -> None:
        if self.bucket_lazy[k] == self.id:
            return
    ...
    def __getitem__(self, k: int) -> T:
        ...
        return (
            self.data[p][k - p * self.bucket_size]
            if self.bucket_lazy[p] == self.id
            else self.mapping(
                self.bucket_lazy[p], self.data[p][k - p * self.bucket_size]
            )
        )
