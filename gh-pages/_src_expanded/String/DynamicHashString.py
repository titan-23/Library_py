# from Library_py.String.DynamicHashString import DynamicHashString
# ref: https://qiita.com/keymoon/items/11fac5627672a6d6a9f6
# from Library_py.DataStructures.SplayTree.ReversibleLazySplayTree import ReversibleLazySplayTreeData, ReversibleLazySplayTree
...
class ReversibleLazySplayTree(Generic[T, F]):
  ...
  def _propagate(self, node: int) -> None:
    ...
    if arr[node<<2|3]:
      ...
      keydata[node*3+1], keydata[node*3+2] = keydata[node*3+2], keydata[node*3+1]
  ...
  def _update(self, node: int) -> None:
    ...
    keydata[node*3+1] = data.op(data.op(keydata[lnode*3+1], keydata[node*3+0]), keydata[rnode*3+1])
    keydata[node*3+2] = data.op(data.op(keydata[rnode*3+2], keydata[node*3+0]), keydata[lnode*3+2])
...
_titan23_DynamicHashString_MASK61: Final[int] = _titan23_DynamicHashString_MOD
...
class DynamicHashStringBase():
  ...
  def get_mul(a: int, b: int) -> int:
    au = a >> 31
    ad = a & _titan23_DynamicHashString_MASK31
    bu = b >> 31
    bd = b & _titan23_DynamicHashString_MASK31
    mid = ad * bu + au * bd
    midu = mid >> 30
    midd = mid & _titan23_DynamicHashString_MASK30
  ...
  def get_mod(x: int) -> int:
    # 商と余りを計算して足す->割る
    xu = x >> 61
    xd = x & _titan23_DynamicHashString_MASK61
    res = xu + xd
    if res >= _titan23_DynamicHashString_MOD:
      res -= _titan23_DynamicHashString_MOD
    return res
...
class DynamicHashString():
  ...
  def set(self, k: int, c: str) -> None:
    self.splay[k] = (_titan23_DynamicHashString_DIC[c], 1)
