# from Library_py.DataStructures.AVLTree.ReversibleLazyAVLTree import ReversibleLazyAVLTree
...
class ReversibleLazyAVLTree(Generic[T, F]):
  ...
  class Node():
    ...
    def __init__(self, key: T, id: F):
      ...
      self.data: T = key
      self.rdata: T = key
      self.left: Optional[ReversibleLazyAVLTree.Node] = None
      self.right: Optional[ReversibleLazyAVLTree.Node] = None
      self.lazy: F = id
      self.rev: int = 0
      self.height: int = 1
      self.size: int = 1
    ...
    def __str__(self):
      if self.left is None and self.right is None:
        return f'key:{self.key, self.height, self.size, self.data, self.rdata, self.lazy, self.rev}\n'
      return f'key:{self.key, self.height, self.size, self.data, self.rdata, self.lazy, self.rev},\n left:{self.left},\n right:{self.right}\n'
  ...
  def __init__(self,
    ...
    self.root: Optional[ReversibleLazyAVLTree.Node] = node
  ...
  def _propagate(self, node: Node) -> None:
    ...
    if node.lazy != self.id:
      ...
      if l:
        ...
        l.rdata = self.mapping(lazy, l.rdata)
      ...
      if r:
        ...
        r.rdata = self.mapping(lazy, r.rdata)
  ...
  def split(self, k: int) -> Tuple['ReversibleLazyAVLTree', 'ReversibleLazyAVLTree']:
    ...
    return ReversibleLazyAVLTree([], self.op, self.mapping, self.composition, self.e, self.id, l), ReversibleLazyAVLTree([], self.op, self.mapping, self.composition, self.e, self.id, r)

  def insert(self, k: int, key: T) -> None:
    s, t = self._split_node(self.root, k)
    self.root = self._merge_with_root(s, ReversibleLazyAVLTree.Node(key, self.id), t)
  ...
  def all_apply(self, f: F) -> None:
    ...
    self.root.rdata = self.mapping(f, self.root.rdata)
  ...
  def prod(self, l: int, r: int) -> T:
    ...
    def dfs(node: ReversibleLazyAVLTree.Node, left: int, right: int) -> T:
