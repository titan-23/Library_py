# from Library_py.DataStructures.SplayTree.SplayTreeDict import SplayTreeDict
...
class SplayTreeDict(Generic[T]):
  ...
  def _set_search_splay(self, key: T) -> None:
    ...
    while keys[node] != key:
      d = key > keys[node]
      if not child[node<<1|d]: break
      if (d and key > keys[child[node<<1|1]]) or (d^1 and key < keys[child[node<<1]]):
        new = child[node<<1|d]
  ...
  def __delitem__(self, key: T) -> None:
    if self._root == 0: return
    self._set_search_splay(key)
    if self._keys[self._root] != key: return
